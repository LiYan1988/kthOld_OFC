...
for i in range(20):
    ...
    dst = 'arch2_pod125_'+str(i)+'.py'
    dst = os.path.join(dirname, dst)
    
    newline = "    with open('simu2_matrix_"+str(i)+".csv') as f:\n"
...
for i in range(20):
    ...
    newline13 = "pdcp arch2_pod125_"+str(i)+".py simu2_matrix_"+str(i)+".csv $TMPDIR\n"
    newline17 = "python arch2_pod125_"+str(i)+".py\n"
    destination = open( dst, "w" )
    source = open( src, "r" )
    for l, line in enumerate(source):
        if l==3:
