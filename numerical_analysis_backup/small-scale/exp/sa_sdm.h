...
Tensor3d_bool getres(ConnectionList connection_list, int num_pods,
    int num_cores, int num_specs, int archidx)
{
    Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
            Row_bool(num_specs, true)));
    if(archidx==2){
        ...
        for(auto c: in_list){
            first_fit(c, res, num_cores, num_specs);
        }
        return res;
    }
    else if(archidx==5){
...
//            for(auto i:energies) cout<<i<<endl;
    ...
    /*********************  Ajmal benchmark  ************************/
    ConnectionList bench_ajmal_arch4(ConnectionList in_list, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
        ...
        while(remain_list.size()>0){
            ...
            for(int i=0; i<tmp_list.size(); i++){
                ...
                else order_list.push_back(tmp_list[i]);
            }
            if(!flag){
    ...
    ConnectionList bench_ajmal_arch4_new(ConnectionList in_list, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
    ...
    ConnectionList bench_ajmal_arch2(ConnectionList in_list, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
    ...
    ConnectionList bench_ajmal_arch2_new(ConnectionList in_list, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
    ...
    ConnectionList bench_ajmal_arch5(ConnectionList in_list, int msize, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
    ...
    ConnectionList bench_ajmal_arch5_new(ConnectionList in_list, int msize, int&energy)
    {
        Tensor3d_bool res(num_pods, Matrix_bool(num_cores,
                  Row_bool(num_specs, true)));
        sort(in_list.begin(), in_list.end());
...
void write_list(ConnectionList inputlist, string listfile)
{
    ofstream myfile;
    myfile.open(listfile);
    myfile<<"source POD, destination POD, #cores, #slots,"<<endl;
    for(auto c:inputlist){
        myfile<<c.src<<","<<c.dst<<","<<c.nc<<","<<c.ns<<","<<endl;
    }
    myfile.close();
}

ConnectionList arch4_list(ConnectionList inputlist, int num_cores)
{
    ConnectionList outputlist;
    for(auto c: inputlist){
        Connection tmpc(c.src, c.dst, c.total_slots, c.gb);
        tmpc.change_core(num_cores);
        outputlist.push_back(tmpc);
    }
    return outputlist;
}

ConnectionList arch5_list(ConnectionList inputlist, int num_pods,
    int num_cores, int num_specs)
{
    Tensor3d_bool res(num_pods, Matrix_bool(num_cores, Row_bool(num_specs, true)));
    ConnectionList remain_list;
    ConnectionList outputlist;
    for(auto c: inputlist){
        bool tmp = first_fit(c, res, num_cores, num_specs);
        if(!tmp) remain_list.push_back(c);
        else outputlist.push_back(c);
    }
    for(auto c: remain_list){
        Row tmpc = c.possible_cores;
        bool tmp;
...
void write3dres(const Tensor3d_bool &res, string filename)
{
    int num_pods = res.size();
    int num_cores = res[0].size();
    int num_specs = res[0][0].size();
    ofstream myfile;
    myfile.open(filename);
    myfile<<"num_pods,"<<num_pods<<endl;
    myfile<<"num_cores,"<<num_cores<<endl;
    myfile<<"num_specs,"<<num_specs<<endl;
