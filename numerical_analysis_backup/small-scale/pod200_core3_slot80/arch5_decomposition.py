...
class Arch5_decompose(object):
    ...
    def create_model_routing(self, **kwargs):
        ...
        is_suc = {}
        for u in self.traffic_pairs:
            is_suc[u] = model_routing.addVar(vtype=GRB.BINARY, obj=-(self.alpha+self.beta*tm[u[0],u[1]]))
    ...
    def create_model_sa(self, **kwargs):
        ...
        isfail = {}
        for u in self.suc_set:
            isfail[u] = model_sa.addVar(vtype=GRB.BINARY, obj=(self.alpha+self.beta*tm[u[0],u[1]]))
...
#    tm = tm.as_matrix()*25
    ...
    #%% optimize    
    m = Arch5_decompose(tm, num_slots=num_slots, num_cores=num_cores, alpha=1, beta=0.001)
