...
// tuple<int, int, int> check_deadline_arch5(const Connection&c, Tensor3d_bool res, 
//     int num_cores, int num_specs)
// {
//     int src = c.src;
//     int dst = c.dst;
...
// bool first_fit_arch5(Connection&c, Tensor3d_bool&res, 
//     int num_cores, int num_specs)
// {
//     int src = c.src;
//     int dst = c.dst;
...
//     int deadline;
//     Connection tmpc{c};
//     tmpc.change_core(tmpc.possible_cores[coreidx]);
//     tuple<int, int, int> besttuple = check_deadline_arch5(tmpc, res, num_cores, num_specs);
//     if(get<0>(besttuple)!=-1) deadline = get<0>(besttuple)+c.total_slots-1;
//     else deadline = num_specs;
//     // other #cores?
//     int deadline_new;
//     tuple<int, int, int> tmptuple;
//     for(int i=1; i<c.possible_cores.size(); ++i){
//         tmpc.change_core(tmpc.possible_cores[i]);
//         tmptuple = check_deadline_arch5(tmpc, res, num_cores, num_specs);
//         if(get<0>(besttuple)!=-1) deadline_new = get<0>(besttuple)+tmpc.ns-1;
//         else deadline_new = num_specs;
...
//         }
//     }
//     if(get<0>(besttuple)==-1) return false;
//     else{
//         c.change_core(c.possible_cores[coreidx]);
//         update_resource(res, get<0>(besttuple), get<1>(besttuple),  get<2>(besttuple), tmpc);
//         return true;
//     }
// }
...
// void Energy_arch1(ConnectionList connection_list, int num_cores, 
...
//            for(auto i:energies) cout<<i<<endl;
    ...
    ConnectionList sa_arch1_is(ConnectionList in_list, bool write_head)
    {
        // simulated annealing process
        s_cur = 0;
        T_cur = Tmax;
...
//            for(auto i:energies) cout<<i<<endl;
    ...
    ConnectionList sa_arch4_is(ConnectionList in_list, bool write_head)
    {
        // simulated annealing process
        s_cur = 0;
        T_cur = Tmax;
...
//            for(auto i:energies) cout<<i<<endl;
    ...
    ConnectionList sa_arch5_is(ConnectionList in_list, bool write_head)
    {
        // simulated annealing process
        s_cur = 0;
        T_cur = Tmax;
