...
tm = np.array(tm)
#%% arch2
betav = np.array([1e-4, 2e-4, 4e-4, 8e-4, 
                        2e-3, 4e-3, 8e-3, 
                        2e-2, 4e-2, 8e-2, 
                        2e-1, 4e-1, 1])
#betav = np.array([2e-3, 4e-3])
connection_ub = []
throughput_ub = []
obj_ub = []
...
for beta in betav:        
    m = Arch2_decompose(tm, num_slots=num_slots, num_cores=num_cores, 
        alpha=1,beta=beta, num_guard_slot=0) 

    m.create_model_routing(mipfocus=1,timelimit=7200,mipgap=0.1, method=2)
...
#    m.write_result_csv('cnklist_lb_%d_%d.csv'%(i,c), m.cnklist_lb)
    ...
    m.write_result_csv('cnklist_heuristic_i%d_c%.2e.csv'%(i,beta), m.cnklist_heuristic_)
