...
cnkUbA2G0 = 1-mean(connectionUb./connectionTotal, 2);
thpUbA2G0 = 1-mean(throughputUb./throughputTotal, 2);
objUbA2G0 = mean(objUb, 2);
cnkHeA2G0 = 1-mean(connectionHeu./connectionTotal, 2);
thpHeA2G0 = 1-mean(throughputHeu./throughputTotal, 2);
objHeA2G0 = mean(objHeu, 2);
...
hold(axes1, 'on')
grid(axes1, 'on')
h(1) = plot(corev,thpUbA2G0, '--', ...
    'displayname', 'Optimal throughput', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]);
h(2) = plot(corev,thpHeA2G0, ...
...
hold(axes2, 'on')
grid(axes2, 'on')
h(1) = plot(corev,cnkUbA2G0, '--', ...
    'displayname', 'Optimal connection', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]);
h(2) = plot(corev,cnkHeA2G0, ...
    'displayname', 'Heuristic connection', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]); % [.93, .69, .13]

save('arch2Guard0beta100.mat', 'cnkUbA2G0', 'thpUbA2G0', 'objUbA2G0', ...
    'cnkHeA2G0', 'thpHeA2G0', 'objHeA2G0')
