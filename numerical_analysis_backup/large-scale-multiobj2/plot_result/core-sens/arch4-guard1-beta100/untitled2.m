...
cnkUbA4G1 = 1-mean(connectionUb./connectionTotal, 2);
thpUbA4G1 = 1-mean(throughputUb./throughputTotal, 2);
objUbA4G1 = mean(objUb, 2);
cnkHeA4G1 = 1-mean(connectionHeu./connectionTotal, 2);
thpHeA4G1 = 1-mean(throughputHeu./throughputTotal, 2);
objHeA4G1 = mean(objHeu, 2);
...
hold(axes1, 'on')
grid(axes1, 'on')
h(1) = plot(corev,thpUbA4G1, '--', ...
    'displayname', 'Optimal throughput', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]);
h(2) = plot(corev,thpHeA4G1, ...
...
hold(axes2, 'on')
grid(axes2, 'on')
h(1) = plot(corev,cnkUbA4G1, '--', ...
    'displayname', 'Optimal connection', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]);
h(2) = plot(corev,cnkHeA4G1, ...
    'displayname', 'Heuristic connection', 'linewidth', 2, ...
    'color', [0, 0.45, 0.74]); % [.93, .69, .13]

save('arch4Guard1beta100.mat', 'cnkUbA4G1', 'thpUbA4G1', 'objUbA4G1', ...
    'cnkHeA4G1', 'thpHeA4G1', 'objHeA4G1')
