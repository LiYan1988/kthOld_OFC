    ...
    def try_channels(self, src, dst, tfk, restensor, ncores):
        ...
        for src_group, dst_group in group_candidates:
            ...
            if tmpidx.size:
                ...
                cnktmp = [src, dst, spec_idx, nslots, src_cores[0], 
                          dst_cores[0], len(src_cores), self.tm[src,dst]]
                self.cnklist_ff.append(cnktmp)
...
if __name__=='__main__':
    ...
    #%% optimize
    m = Arch5_decompose(tm, num_slots=num_slots, num_cores=num_cores, alpha=1, beta=0.0)
    m.create_model_routing(mipfocus=1, timelimit=20, method=2, mipgap=0.01)
#    m.create_model_sa(mipfocus=1,timelimit=1000, method=2, SubMIPNodes=2000, 
#                      Heuristics=0.8)
#    print m.connection_lb_/float(m.connection_ub_)
#    print m.throughput_lb_/float(m.throughput_ub_)
    m.heuristic()
