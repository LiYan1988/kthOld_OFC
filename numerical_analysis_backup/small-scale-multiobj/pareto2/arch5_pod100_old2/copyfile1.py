...
for i in range(20):
    ...
    dst = 'pareto1_'+str(i)+'.sh'
    dst = os.path.join(dirname, dst)
    
    newline3 = "#SBATCH -J arch5_old1_"+str(i)+"\n"
    newline6 = "#SBATCH -o arch5_old1_"+str(i)+".stdout\n"
    newline7 = "#SBATCH -e arch5_old1_"+str(i)+".stderr\n"
    newline17 = "pdcp pareto1_"+str(i)+".py $TMPDIR\n"
    newline21 = "python pareto1_"+str(i)+".py\n"
    destination = open( dst, "w" )
    source = open( src, "r" )
    for l, line in enumerate(source):
        if l==3:
...
f = open('run_sbatch1.txt', 'w')
for i in range(20):
    line = 'sbatch pareto1_'+str(i)+'.sh\n'
    f.write(line)
f.close()