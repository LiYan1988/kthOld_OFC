# -*- coding: utf-8 -*-
"""
Created on Thu Sep  8 23:01:50 2016
...
#                print(traffic_slot)
    ...
    def write_heuristic_result_csv(self, file_name):
        with open(file_name, 'w') as f:
            f.write("src,dst,spec,used_slot,tfk_slot\n")
            for u in self.heuristic_resource:
                wstr = "{},{},{},{},{}\n".format(u[0], u[1], u[2], u[3], 
                    self.traffic_capacities[u[0],u[1]])
                f.write(wstr)
    ...
    def sa_heuristic(self, ascending1=False, ascending2=True):
        ...
        for i in range(len(IS_list)):
            for u in IS_list[i]:
                ...
                if tmpidx.size:
                   ...
                   suclist.append(u)
                   self.heuristic_resource.append([src, dst, spec_idx, self.traffic_capacities[u]])
        ...
        for u in remain_cnk:
            ...
            if avail_slots[0][1][1]<=self.num_slots:
                ...
                self.obj_sah_throughput_ += self.tm[src,dst]
                suclist.append(u)
                self.heuristic_resource.append([src, dst, spec_idx, spec_bd])
