...
for beta in betav:        
    ...
    m.create_model_routing(mipfocus=1,timelimit=600,mipgap=0.01, method=2, SubMIPNodes=2000, heuristics=0.8)
    m.create_model_sa(mipfocus=1,timelimit=300, method=2, SubMIPNodes=2000, heuristics=0.8)
    ...
    connection_lb.append(m.connections_lb)
    throughput_lb.append(m.throughput_lb)
    obj_lb.append(m.alpha*m.connections_lb+m.beta*m.throughput_lb)
...
file_name = "result_pareto_arch5_pod100_nf_{}.csv".format(i)
