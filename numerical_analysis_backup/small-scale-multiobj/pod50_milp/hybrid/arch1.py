...
class ModelSDM_arch1(object):
    ...
    def create_model(self, **kwargs):
        ...
        bin_pair = {}
        for i, j in traffic_pairs:
            bin_pair[i, j] = m1.addVar(vtype=GRB.BINARY, obj=-(self.alpha+self.beta*self.tm_arch[i,j]),
        ...
        self.connections = sum(bin_pair[u].x for u in traffic_pairs)
        self.throughput = sum(bin_pair[u].x*self.tm_arch[u[0],u[1]] for u in traffic_pairs)
        
        # a list of successfully allocated connections        
        self.suclist = []
        for u in traffic_pairs:
            if bin_pair[u].x == 1:
                self.suclist.append(u)
        # a dict of allocated resources
        self.core_usagex = {}
        for u in self.suclist:
            self.core_usagex[u,u[0]] = int(sum(k*bin_pair_core_out[u[0],u[1],k].x for k in cores))
            self.core_usagex[u,u[1]] = int(sum(k*bin_pair_core_in[u[0],u[1],k].x for k in cores))
    ...
    def write_result_csv(self, file_name):
        with open(file_name, 'w') as f:
            ...
            writer.writerow(['src', 'dst', 'core_src', 'core_dst'])
            for u in self.suclist:
                writer.writerow([u[0], u[1], self.core_usagex[u,u[0]],self.core_usagex[u,u[1]]])
...
if __name__=='__main__':
    ...
    #%% solve
    m = ModelSDM_arch1(traffic_matrix, num_slots=num_slots, num_cores=num_cores, alpha=0, beta=1)
#    m.create_model1(mipfocus=1, timelimit=10)
    m.create_model(mipfocus=1, timelimit=100)
    m.write_result_csv('arch1.csv')
