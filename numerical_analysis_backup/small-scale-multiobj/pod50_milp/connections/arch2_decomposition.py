...
#        for i in self.pods:
#            for k in self.cores:
#                cnklist = list(self.pcset[i,k])
#                for r in range(len(cnklist)):
#                    for s in range(r+1, len(cnklist)):
#                        for t in range(s+1, len(cnklist)):
#                            model_sa.addConstr(spec_order[])
...
#    tm = tm.as_matrix()*25
    ...
    m.create_model_sa(mipfocus=1,timelimit=2000)
