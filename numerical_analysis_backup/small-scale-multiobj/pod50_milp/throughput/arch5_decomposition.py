...
class Arch5_decompose(object):
    ...
    def create_model_sa(self, **kwargs):
        ...
        try:
            suclisttmp = list(self.suclist)
            for u in self.suclist:
                if isfail[u].x==1:
                    suclisttmp.remove(u)
            self.suclist = suclisttmp
...
#    tm = tm.as_matrix()*25
    ...
    #%% optimize    
    m = Arch5_decompose(tm, num_slots=num_slots, num_cores=num_cores, alpha=1, beta=0.02)
    m.create_model_routing(mipfocus=1, timelimit=100, mipgap=0.01)
    m.create_model_sa(mipfocus=1, timelimit=400, mipgap=0.01)
