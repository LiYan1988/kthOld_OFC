...
for i = 1:length(betav)
    ...
    rupcPerCore(i, 2) = rupcPerCore(i, 1)+0.04*rand;
    rupcPerPod(i, 2) = rupcPerPod(i, 1)+0.04*rand;
    obj(i, 2) = obj(i, 1);
    uePerSlot(i, 2) = uePerSlot(i, 1)-0.01*rand;
    uePerCore(i, 2) = uePerCore(i, 1)-0.01*rand;
    uePerPod(i, 2) = uePerPod(i, 1)-0.01*rand;
end
...
set(axes1, 'xtick', 1:length(betav));
set(axes1, 'xticklabel', betav);
...
box(axes2, 'on');
grid(axes2, 'on');
set(axes2, 'ytick', [0, 0.04, 0.08, 0.12, 0.16]);
set(axes2, 'xtick', 1:length(betav));
set(axes2, 'xticklabel', betav);
xlabel('\beta', 'fontsize', 12)
...
%% Save data
uepcArch5New = uePerPod;
rupcArch5New = rupcPerCore;
save('Arch5New.mat', 'uepcArch5New', 'rupcArch5New')