...
resultAve(:,5) = smooth(resultAve(:,1), resultAve(:,5));
resultAve(:,6) = smooth(resultAve(:,1), resultAve(:,6));
resultAve(:,2) = resultAve(:,2)*0.95;
resultAve(:,3) = resultAve(:,3)*0.95;
%%
figure1 = figure;
...
h = legend(h(1:3), 'location', 'best');
h.FontSize = 12;
saveas(figure1, 'pareto_arch2.jpg')

%%

betam = resultAve(1,1)/10;
figure2 = figure;
...
h(3) = semilogx(resultAve(:,1)+betam, resultAve(:,3)*0.001*0.88, '--', 'linewidth', 2, 'displayname', '88% of optimal');
box(axes2, 'on')
grid(axes2, 'on')
