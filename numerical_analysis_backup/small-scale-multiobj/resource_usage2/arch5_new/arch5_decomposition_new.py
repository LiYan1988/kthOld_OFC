...
class Arch5_decompose(object):
    ...
    def create_model_routing(self, **kwargs):
        ...
        self.channels_core_nslot = channels_core_nslot
        ...
        for u in self.traffic_pairs:
            for i in channels_core:
                core_choice[u,u[0],i] = model_routing.addVar(vtype=GRB.BINARY, 
                    name='core_choice_(%d,%d)%d_%d'%(u[0],u[1],u[0],i))
                core_choice[u,u[1],i] = model_routing.addVar(vtype=GRB.BINARY, 
                    name='core_choice_(%d,%d)%d_%d'%(u[0],u[1],u[1],i))
                
        is_suc = {}
        for u in self.traffic_pairs:
            is_suc[u] = model_routing.addVar(vtype=GRB.BINARY, 
                obj=-(self.alpha+self.beta*self.tm[u[0],u[1]]),
                name='is_suc_(%d,%d)'%(u[0],u[1]))
            
        flow_core = {}
        for i in self.pods:
            for j in self.cores:
                flow_core[i,j] = model_routing.addVar(vtype=GRB.CONTINUOUS, 
                    ub=self.num_slots, name='flow_core_%d_%d'%(i,j))
        ...
        self.obj_ub = self.alpha*self.connections_ub+self.beta*self.throughput_ub  
    ...
    def create_model_sa(self, **kwargs):
        ...
        self.obj_lb = self.alpha*self.connections_lb+self.beta*self.throughput_lb
    ...
    def sa_heuristic(self, ascending1=False, ascending2=True, **kwargs):
        """
        """
        # modify routing model        
        model_routing = self.model_routing.copy()
        core_choice = {}
        for u in self.traffic_pairs:
            for i in self.channels_core:
                core_choice[u,u[0],i] = model_routing.getVarByName(
                    name='core_choice_(%d,%d)%d_%d'%(u[0],u[1],u[0],i))
                core_choice[u,u[1],i] = model_routing.getVarByName(
                    name='core_choice_(%d,%d)%d_%d'%(u[0],u[1],u[1],i))
                    
        is_suc = {}
        for u in self.traffic_pairs:
            is_suc[u] = model_routing.getVarByName(
                name='is_suc_(%d,%d)'%(u[0],u[1]))
#            is_suc[u].obj = 0
#            is_suc[u].lb = 1
        ...
        for i in self.pods:
            for j in self.cores:
                flow_core[i,j] = model_routing.getVarByName(
                    name='flow_core_%d_%d'%(i,j))
                flow_core[i,j].ub = self.num_slots*1.1
#                flow_core[i,j].obj = 1
        ...
        nslot_choice = {} # number of spectral slots per core for connection u using channel i
        for u in self.traffic_pairs:
            core_choicex[u,u[0]] = int(np.round(sum(i*core_choice[u,u[0],i].x 
                for i in self.channels_core)))
            core_choicex[u,u[1]] = int(np.round(sum(i*core_choice[u,u[1],i].x 
                for i in self.channels_core)))
            nslot_choice[u] = self.channels_core_nslot[u,core_choicex[u,u[0]]]
                        
        core_usagex = {}
        for u in self.traffic_pairs:
            chout = core_choicex[u,u[0]]
            chin = core_choicex[u,u[1]]
            core_out = np.where(self.B[:,chout]==1)[0]
            core_in = np.where(self.B[:,chin]==1)[0]
            core_usagex[u,u[0]] = core_out
            core_usagex[u,u[1]] = core_in
        ...
        for i in self.pods:
            ...
            for j in self.cores:
                cnk_in_core[i,j] = []
                for u in tmp:
                    if sum(core_choice[u,i,k].x*self.B[j,k]for k in self.channels_core)==1:
                        cnk_in_core[i,j].append(u)       
            
        suclist = [u for u in self.traffic_pairs if is_suc[u].x==1]
        
        suclist_tm = [nslot_choice[u] for u in self.traffic_pairs]
        if ascending1:
        ...
        while len(suclist):
            ...
            for u in tmplist:
                src = u[0]
                dst = u[1]
                src_core = list(core_usagex[u,src])
                dst_core = list(core_usagex[u,dst])
        ...
        for i in range(len(IS_list)):
            for u in IS_list[i]:
                src = u[0]
                dst = u[1]
                src_core = core_usagex[u,src]
                dst_core = core_usagex[u,dst]
                ...
                tmpidx = np.where(tmpavail[:,1]>=nslot_choice[u])[0]
                if tmpidx.size:
                   spec_idx = tmpavail[tmpidx[0],0]
                   restensor[src,src_core,spec_idx:(spec_idx+nslot_choice[u])] = False
                   restensor[dst,dst_core,spec_idx:(spec_idx+nslot_choice[u])] = False
                   self.obj_sah_ += self.alpha+self.beta*self.tm[src,dst]
                   self.obj_sah_connection_ += 1
                   self.obj_sah_throughput_ += self.tm[src,dst]
                   suclist_allocated.append(u)

        remain_cnk = [u for u in self.traffic_pairs if u not in suclist_allocated]
        ...
        for u in remain_cnk:
            ...
            spec_bd = int(self.tm[u]*self.slot_capacity+self.num_guard_slot)
            k = 0
            avail_slots = {}
            for ksrc in self.cores:
                for kdst in self.cores:
                    ...
                    tmpidx = np.where(tmpavail[:,1]>=spec_bd)[0]
                    if not tmpidx.size:
        ...
        self.suclist_heuristic = suclist_allocated
...
if __name__=='__main__':
    ...
    #%% generate traffic
    num_pods=100
    max_pod_connected=int(num_pods*0.2)
    min_pod_connected=1
    mean_capacity=200
    ...
    m.create_model_sa(mipfocus=1,timelimit=100, method=2, SubMIPNodes=2000, Heuristics=0.8)
#    print m.connections_lb/float(m.connections_ub)
#    print m.throughput_lb/float(m.throughput_ub)
#    m.sa_heuristic(ascending1=True, ascending2=False, SubMIPNodes=2000, Heuristics=0.8,
#                   mipfocus=1, timelimit=1000, method=2, mipgap=0.015)
#    print float(m.obj_sah_connection_)/m.connections_ub
#    print float(m.obj_sah_throughput_)/m.throughput_ub
#    print m.obj_sah_/m.obj_lb
#    print m.obj_sah_/m.obj_ub
    print m.obj_lb/m.obj_ub