...
betav = [0, 1e-5, 1e-4, 2e-4, 5e-4, 1e-3, 3e-3, 3e-2, 0.1, 1]
dirname=''
for i, b in enumerate(betav):
    src = os.path.join(dirname, 'template.py')
    dst = 'ru_arch5_'+str(i)+'.py'
...
for i,b in enumerate(betav):
    ...
    dst = 'ru_arch5_'+str(i)+'.sh'
    dst = os.path.join(dirname, dst)
    
    newline3 = "#SBATCH -J ru_arch5_new_"+str(i)+"\n"
    newline6 = "#SBATCH -o ru_arch5_new_"+str(i)+".stdout\n"
    newline7 = "#SBATCH -e ru_arch5_new_"+str(i)+".stderr\n"
    newline17 = "pdcp ru_arch5_"+str(i)+".py $TMPDIR\n"
    newline21 = "python ru_arch5_"+str(i)+".py\n"
    destination = open( dst, "w" )
    source = open( src, "r" )
    for l, line in enumerate(source):
        if l==3:
...
for i, b in enumerate(betav):
    line = 'sbatch ru_arch5_'+str(i)+'.sh\n'
    f.write(line)
f.close()