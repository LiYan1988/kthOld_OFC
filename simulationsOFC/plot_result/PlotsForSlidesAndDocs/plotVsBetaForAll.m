...
yyaxis(axes2, 'left')
% h(1) = semilogx(paretoArch2Old(:,5)*200, paretoArch2Old(:,1), 'linewidth', 2, ...
%     'displayname', 'throughput, bound');
hold(axes2, 'on')
h(2) = semilogx(paretoArch2Old(:,5)*200, paretoArch2Old(:,3), 'linewidth', 2, ...
    'displayname', 'throughput, heuristic');
ylabel('Throughput (Tbps)', 'fontsize', 16)

yyaxis(axes2, 'right')
% h(3) = semilogx(paretoArch2Old(:,5)*200, paretoArch2Old(:,2), 'linewidth', 2, ...
%     'displayname', 'connection, bound');
hold(axes2, 'on')
h(4) = semilogx(paretoArch2Old(:,5)*200, paretoArch2Old(:,4), 'linewidth', 2, ...
