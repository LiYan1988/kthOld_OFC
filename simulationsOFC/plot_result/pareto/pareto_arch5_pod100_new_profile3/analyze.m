...
for i=0:19
    filename = sprintf('result_pareto_arch5_pod100_nf_%d.csv',i);
    resultSmallBeta(:,:,i+1) = importfile_pareto(filename);
end

resultBigBeta = zeros(size(resultSmallBeta));
for i=0:19
    filename = sprintf('1_result_pareto_arch5_pod100_nf_%d.csv',i);
...
% resultAve(:,2) = smooth(resultAve(:,1), resultAve(:,2), 11);
% resultAve(:,3) = smooth(resultAve(:,1), resultAve(:,3), 11);
...
h(3) = plot(resultAve(1:end-28,2)*0.99,resultAve(1:end-28,3)*0.001*0.99, '--', 'displayname', '99% of optimal', 'linewidth', 2);

paretoArch5New(:,1) = resultAve(1:end-28,2);
paretoArch5New(:,2) = resultAve(1:end-28,3)*0.001;
paretoArch5New(:,3) = resultAve(1:end-28,5);
paretoArch5New(:,4) = resultAve(1:end-28,6)*0.001;
save('paretoArch5New.mat', 'paretoArch5New')
...
h(3) = semilogx(resultAve(:,1), resultAve(:,3)*0.001*0.99, '--', 'linewidth', 2, 'displayname', '99% of optimal');
box(axes2, 'on')
grid(axes2, 'on')
