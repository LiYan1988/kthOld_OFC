...
for i=0:19
    ...
    result(:,:,i+1) = importPareto(filename);
end
...
h(3) = plot(resultAve(:,2)*0.85,resultAve(:,3)*0.001*0.85, '--', 'displayname', '90% of optimal', 'linewidth', 2);

paretoArch2New(:,1) = resultAve(:,2);
paretoArch2New(:,2) = resultAve(:,3)*0.001;
paretoArch2New(:,3) = resultAve(:,8);
paretoArch2New(:,4) = resultAve(:,9)*0.001;
save('paretoArch2New.mat', 'paretoArch2New')
...
h = legend(h(1:3), 'location', 'northwest');
h.FontSize = 12;
...
h(3) = semilogx(resultAve(:,1), resultAve(:,3)*0.001*0.85, '--', 'linewidth', 2, 'displayname', '90% of optimal');
