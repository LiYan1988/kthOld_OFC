...
for i = 1:20
    for j = 1:length(betav)
        ...
        [src,dst,spec,slots_used,core_src,core_dst,tfk_slot, cnkMatrix] = ...
            importCnk(fileName);
        [matrixResource, efficiencies(i, j)] = ...
...
ueAveVsBetaA2 = mean(utilizationEntropyAve, 1);
ueStdVsBetaA2 = mean(utilizationEntropyStd, 1);
efVsBetaA2 = mean(efficiencies, 1);
ruVsBetaA2 = mean(resourceUsageAve, 1);
...
h(1) = semilogx(betav, ueAveVsBetaA2, 'linewidth', 2, ...
...
h(1) = semilogx(betav, ruVsBetaA2, 'linewidth', 2, ...
...
save('arch2RU.mat', 'ruVsBetaA2', 'efVsBetaA2', 'ueAveVsBetaA2', 'efVsBetaA2')
